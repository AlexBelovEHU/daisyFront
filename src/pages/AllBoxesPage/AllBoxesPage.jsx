...
import AllBoxes from "../../components/AllBoxesPage/AllBoxes";
import LoadingView from "../../components/LoadingView/LoadingView";

function AllBoxesPage() {
  ...
  const [currentBoxesnum, setBoxesnum] = React.useState(12);
  ...
  const increaseBoxesnum = () => {
    setBoxesnum(currentBoxesnum + 12);
  };
  ...
  const openBonusBox = () => {
    if (bonusBoxes.length != 0) {
      const boxinfo = featuredboxes.filter(function (item) {
        return item.id == bonusBoxes[0];
      })[0];
    ...
    if (characters[0] == "NotLoggedIn") {
      ...
      if (payloadType == "BALANCE_GET") {
        ...
        if (characters.length != 0) {
          ...
          let bronze = JSON.stringify(characters.filter(function (item) { return item.id == window.clientConfig.bronzeBoxId; }))
          if (bronze != []) {
            localStorage.setItem("bronzeBox",bronze);            
          }
          let silver = JSON.stringify(characters.filter(function (item) { return item.id == window.clientConfig.silverBoxId; }))
          if (silver != []) {
            localStorage.setItem("silverBox",silver);         
          }
          let gold = JSON.stringify(characters.filter(function (item) { return item.id == window.clientConfig.goldBoxId; }))
          if (gold != []) {
